...
if (s.statementType()==Statement::STATEMENT_SELECT){
    ...
    std::shared_ptr<DataRow> dataRow(new DataRow(static_cast<uint32_t>(std::stoi(dataParts[1])),
                                                 dataParts[2],
                                                 dataParts[3]));
...
SqlFrontend::SqlFrontend() :d_dataTable(std::make_shared<DataTable>(DataTable())){
};
